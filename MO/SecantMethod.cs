...
namespace testing_project
{
    static class SecantMethod
    {
        ...
        static public decimal SecantCount(decimal leftTb, decimal rightTb, decimal tolTb, ProgressBar pb, int maxIter, int time)
        {
            elapsedTime = 0;
            ...
            while (Math.Abs(right - left) > tolTb)
            {
                ...
                decimal leftY = (decimal)F(Convert.ToDouble(left));
                decimal rightY = (decimal)F(Convert.ToDouble(right));
                if (rightY == 0)
                {
                    left = right;
                    break;
                ...
                middle = left + ((-leftY) / (rightY - leftY)) * (right - left);
